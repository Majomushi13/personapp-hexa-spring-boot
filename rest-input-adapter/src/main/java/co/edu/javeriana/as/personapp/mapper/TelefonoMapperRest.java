...
public class TelefonoMapperRest {
    ...
    private TelefonoResponse fromDomainToAdapterRest(Phone phone, String database) {
    ...
    public Phone fromAdapterToDomain(TelefonoRequest request) {
        Person owner = new Person(); // Esta línea necesita una lógica real para asignar el propietario
        owner.setIdentification(request.getOwnerId());
        return new Phone(
                request.getNumber(),
                request.getCompany(),
                owner
